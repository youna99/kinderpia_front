...
export const getChatList = async () :Promise<ChatListResponse> => {
  const response = await requestHeader.get(`/api/chatroom/list/102`, { withCredentials: true });
  return response;
};

// 단일 채팅방 조회
export const getChatRoom = async (chatroomId: number):Promise<ChatRoomResponse> => {
  const response = await requestHeader.post(`/api/chatroom`, chatroomId, { withCredentials: true });
  return response;
};

// 채팅 메시지 조회(목록)
export const getChatMessages = async (chatroomId:number) : Promise<ChatMessageResponse> => {
    const response = await requestHeader.post(`/api/chatroom/chatmsg`, chatroomId, { withCredentials: true })
    return response;
};
